...
def writeLanguage(languageCode, defs, f):
    ...
    obj = lang['menuRootGroups']
    f.write(
        to_unicode("const char* RootMenuEntries[" + str(len(obj)) +
                   "] = {\n"))
    ...
    #### Writing Menu Root Groups Descriptions ####
    obj = lang['menuRootGroups']
    f.write(
        to_unicode("const char* RootMenuEntriesDescriptions[" +
                   str(len(obj)) + "] = {\n"))
...
def readVersion():
    ...
    "/../workspace/ASPD100/version.h"),"r") as version_file:
        try: 
...
def read_opts():
    ...
    if len(sys.argv) > 2:
        outFileTranslationC = sys.argv[2]
    else:
        outDir = os.path.relpath(jsonDir + "/../workspace/ASPD100/Core/Src")
        outFileTranslationC = os.path.join(outDir, TRANSLATION_C)
    
    if len(sys.argv) > 3:
        outFileUnitH = sys.argv[3]
    else:
        outDir = os.path.relpath(jsonDir + "/../workspace/ASPD100/Core/Inc")
        outFileUnitH = os.path.join(outDir,UNIT_H)

    if len(sys.argv) > 4:
        raise Exception("Too many parameters!")

    return jsonDir, outFileTranslationC, outFileUnitH
...
def writeTarget(outFileTranslationC, outFileUnitH, defs, langCodes, UnitCodes):
    # Start writing the file
    with io.open(outFileTranslationC, 'w', encoding='utf-8', newline="\n") as f:
        writeStart(f)
...
if __name__ == "__main__":
    try:
        jsonDir, outFileTranslationC, outFileUnitH = read_opts()
    except:
        print("usage: make_translation.py {json dir} {c dir}")
        sys.exit(1)

    #try: buildVersion = readVersion()
    try: buildVersion = "0.2"
    except: print("error: could not get/extract build version"); sys.exit(1)

    print("Build version: " + buildVersion)
    print("Making " + outFileTranslationC + " from " + jsonDir)
    ...
    writeTarget(outFileTranslationC, outFileUnitH, defs, langCodes, UnitCodes)
